...
function extractVideoData(videoElement) {
    ...
    if (!data.duration || !data.viewCount || !data.publishTime) {
        ...
        if (missing.length > 0) {
            console.debug(
                `[Filter] Incomplete extraction for "${data.title}": Missing ${missing.join(", ")}`,
            );
        }
    }
...
function init() {
        ...
        const observer = new MutationObserver((mutations) => {
            ...
            if (hasNewContent) {
                ...
                filterTimeout = setTimeout(() => {
                    console.log(
                        "[Filter] New videos detected, re-running filters...",
                    );
                    runAllFilters();
    ...
    new MutationObserver(() => {
        ...
        if (currentUrl !== lastUrl) {
            lastUrl = currentUrl;
            console.log(
                "[Filter] Page navigation detected, re-running filters...",
            );
